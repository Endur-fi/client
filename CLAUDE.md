...
- **Portfolio RPC**: `NEXT_PUBLIC_PORTFOLIO_RPC` (optional) for `/api/portfolio/*` and holdings aggregation; falls back to `NEXT_PUBLIC_RPC_URL`
...
- `NEXT_PUBLIC_PORTFOLIO_RPC` — optional dedicated RPC for `/portfolio` API aggregation (falls back to `NEXT_PUBLIC_RPC_URL`)
