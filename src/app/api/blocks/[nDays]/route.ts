...
export async function GET(_req: Request, context: any) {
    ...
    const currentBlockMinus1000 = await provider.getBlock(currentBlock.block_number - 1000);
    const avgBlockTimeSeconds = (currentBlock.timestamp - currentBlockMinus1000.timestamp) / 1000;
    
    const promises: Promise<any>[] = [];
   
    // loop across each day we need block for 
    let now = new Date(currentBlock.timestamp * 1000);
    const endTime = new Date(now.getTime() - nDays * 24 * 60 * 60 * 1000);
    while (now >= endTime) {
        now.setDate(now.getDate() - config.gapDays);
        const timestampSeconds = Math.floor(now.getTime() / 1000);
        promises.push(getBlockForTime(currentBlock.block_number, timestampSeconds, avgBlockTimeSeconds));
    }
...
async function getBlockForTime(
    ...
    const blockDiff = Math.floor((nowSeconds - timestampSeconds) / avgBlockTimeSeconds);
