...
const EKUBO_XSTRK_STRK = '0x01f083b98674bc21effee29ef443a00c7b9a500fd92cf30341a3da12c73f2324'
...
export const getEkuboXSTRKSTRKHoldings: DAppHoldingsFn = async (
  ...
  console.log(
    "getEkuboXSTRKSTRKHoldings::info",
    xSTRKHolings,
    STRKHolings
  );
  // const strkAmount = new MyNumber(holdings.estimated_size.toString(), STRK_DECIMALS);
...
export const getSTRKFarmBalanceAtom: DAppHoldingsAtom = atomFamily(
  (blockNumber?: number) => {
    return atom((get) => {
      ...
      const { data: data2, error: error2 } = get(userEkuboXSTRKSTRKBalanceQueryAtom(blockNumber));

      let xSTRKAmount = data?.xSTRKAmount ?? new MyNumber("0", STRK_DECIMALS);
      let STRKAmount = data?.STRKAmount ?? new MyNumber("0", STRK_DECIMALS);

      if (data2) {
        xSTRKAmount = xSTRKAmount.operate('plus', data2?.xSTRKAmount ? data2.xSTRKAmount.toString() : "0");
        STRKAmount = STRKAmount.operate('plus', data2?.STRKAmount ? data2.STRKAmount.toString() : "0");
      }

      console.log("getSTRKFarmBalanceAtom", data?.STRKAmount.toString(), data?.xSTRKAmount.toString(), data2?.STRKAmount.toString(), data2?.xSTRKAmount.toString())
      console.log("getSTRKFarmBalanceAtom2", xSTRKAmount.toString(), STRKAmount.toString())
      return {
        data: {
          xSTRKAmount,
          STRKAmount,
