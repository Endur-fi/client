...
const usernxSTRKBalanceQueryAtom = atomWithQuery((get) => {
  return {
...
const usernxSTRKcBalanceQueryAtom = atomWithQuery((get) => {
  return {
    // current block atom only to trigger a change when the block changes
    queryKey: [
      "usernxSTRKcBalance",
...
const userixSTRKBalanceQueryAtom = atomWithQuery((get) => {
  return {
    // current block atom only to trigger a change when the block changes
    queryKey: [
      "userixSTRKBalance",
...
const userixSTRKcBalanceQueryAtom = atomWithQuery((get) => {
  return {
    // current block atom only to trigger a change when the block changes
    queryKey: [
      "userixSTRKcBalance",
...
const userdxSTRKBalanceQueryAtom = atomWithQuery((get) => {
  return {
    // current block atom only to trigger a change when the block changes
    queryKey: [
      "userdxSTRKBalance",
...
const userLPTokenBalanceQueryAtom = atomWithQuery((get) => {
  return {
    // current block atom only to trigger a change when the block changes
    queryKey: [
      "userLPTokenBalance",
...
export const usernxSTRKBalance = atom((get) => {
  const { data, error } = get(usernxSTRKBalanceQueryAtom);
...
export const usernxSTRKcBalance = atom((get) => {
  const { data, error } = get(usernxSTRKcBalanceQueryAtom);
...
export const userixSTRKBalance = atom((get) => {
  const { data, error } = get(userixSTRKBalanceQueryAtom);
...
export const userixSTRKcBalance = atom((get) => {
  const { data, error } = get(userixSTRKcBalanceQueryAtom);
...
export const userdxSTRKBalance = atom((get) => {
  const { data, error } = get(userdxSTRKBalanceQueryAtom);
...
export const userLPTokenBalance = atom((get) => {
  const { data, error } = get(userLPTokenBalanceQueryAtom);
...
export const userxSTRKNostraBalance = atom((get) => {
  const nxSTRKBal = get(usernxSTRKBalance);
  const nxSTRKcBal = get(usernxSTRKcBalance);
  const ixSTRKBal = get(userixSTRKBalance);
  const ixSTRKcBal = get(userixSTRKcBalance);
  const dxSTRKBal = get(userdxSTRKBalance);
  const lpTokenBal = get(userLPTokenBalance);
